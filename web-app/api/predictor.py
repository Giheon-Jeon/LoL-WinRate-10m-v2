...
# Expected columns for stats (54 columns)
columns = [
...
def preprocess_stats(input_data):
    data = {}
    for col in columns:
        data[col] = float(input_data.get(col, 0.0))
    
    # Automatically compute gold diffs to avoid manual user load
...
def format_prediction_response(lr_prob, rf_prob, xgb_prob):
    return {
        'logistic_regression': {
...
def predict_stats_rate(input_data):
    data = preprocess_stats(input_data)
    ...
    return format_prediction_response(lr_prob, rf_prob, xgb_prob)
